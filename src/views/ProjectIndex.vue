	...
	<el-scrollbar height="calc(100vh - 70px)" v-if="proInfo">
		...
		<el-card body-style="padding:0px">
			<el-descriptions :column="4" border>
				...
				<el-descriptions-item label="创建时间">{{ $date.rTime(proInfo.create_time) }}</el-descriptions-item>
			...
			<el-row :gutter="20">
				<el-col :span="4" v-for="item in proInfo.info">
					<div class="info">
						<el-progress type="dashboard" :width='90' :percentage="100" :stroke-width="4" color="#61c2b2">
		...
		<el-card body-style="padding:0px">
			...
			<el-row>
				<el-col :span="6" v-for="item in proInfo.bugs">
					<div class="info">
						<el-progress type="circle" :width='90' :percentage="100" :stroke-width='4' color="#55aaff">
							<template #default="{ percentage }">
								<div class="percentage-value">
									<b style="color:#00aaff">{{ item.value }}</b>
									<span>个</span>
								</div>
								<span class="percentage-label">{{ item.name }}</span>
							</template>
